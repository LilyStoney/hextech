...
RSpec.describe Hextech::Adapter, vcr: true do
  describe '#call' do
    subject do
    ...
    context 'without a host method' do
      before do
        allow_any_instance_of(Hextech::Adapter).to receive(:path).and_return('/example')
      end
    ...
    context 'without a path method' do
      before do
        allow_any_instance_of(Hextech::Adapter).to receive(:host).and_return('http://www.example.com')
      end
